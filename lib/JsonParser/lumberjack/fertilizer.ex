defmodule JsonParser.Lumberjack.Fertilizer do
  ...
  defguardp are_siblings?(node1, node2, diff)
  ...
  defp fill_intermediaries(acc, list) when length(list) <= 1,
    do: if(length(acc) > length(list), do: acc, else: list)

  defp check_necessity(node1, node2, diff, list)
       when are_siblings?(elem(node1, 0), elem(node2, 0), diff) do
  ...
  defp iterate(address, start, finish, tokens, tree) do
    ...
    {_, new_tree} =
      get_and_update_in(tree, List.flatten([address, :content]), &nil_killer(&1, content))
  ...
  defp nil_killer(previous_content, new_content) do
    if previous_content == nil do
      ...
      filtered =
        List.flatten([previous_content, new_content])
        |> Enum.dedup()
